document.addEventListener('DOMContentLoaded', () => {
    ...
    analyseBudgetButton.addEventListener('click', async () => {
        ...
        const housingAmount = document.getElementById('???');
        const transportationAmount = document.getElementById('???');
        const leisureAmount = document.getElementById('???');

        budgetData['???'] = parseFloat(foodAmount.value)
        budgetData['???'] = parseFloat(housingAmount.value)
        budgetData['???'] = parseFloat(transportationAmount.value)
